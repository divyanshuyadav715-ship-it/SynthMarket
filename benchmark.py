...
def run_benchmarks():
    results = {}
    print("Starting Exhaustive Benchmarks for SynthMarket...")
...
|-----------------|------------------|-------|-------------|
| **Generative AI** | GAN Distribution Fidelity (KL Divergence) | `{results.get('GAN_KL_Divergence', 0):.4f}` | Lower is better. Measures how closely the synthetic data matches real stock volatility. |
| **RL Trading Agent** | Average Cumulative Return | `{results.get('RL_Avg_Return_Pct', 0):.2f}%` | Return generated by the PPO agent over synthetic episodes. |
| **RL Trading Agent** | Sharpe Ratio (Risk-Adjusted) | `{results.get('RL_Sharpe_Ratio', 0):.2f}` | Higher is better. >1.0 is considered good. |
| **RL Trading Agent** | Maximum Drawdown | `{results.get('RL_Max_Drawdown_Pct', 0):.2f}%` | Lower is better. Measures the largest drop from a peak in portfolio value. |
| **MLOps & API** | PPO Inference Latency | `{results.get('Inference_Latency_ms', 0):.2f} ms` | Time taken for the agent to make a real-time trading decision. |
